...
import { type ShoppingListId, shoppingListIdSchema, shoppingLists } from "@/lib/db/schema/shoppingLists";
...
export const getShoppingListById = async (id: ShoppingListId) => {
  ...
  const [row] = await db.select().from(shoppingLists).where(and(eq(shoppingLists.id, shoppingListId), eq(shoppingLists.userId, session?.user.id!)));
...
export const getShoppingListByIdWithShoppingProducts = async (id: ShoppingListId) => {
  const { session } = await getUserAuth();
  const { id: shoppingListId } = shoppingListIdSchema.parse({ id });
  const rows = await db.select({ shoppingList: shoppingLists, shoppingProduct: shoppingProducts }).from(shoppingLists).where(and(eq(shoppingLists.id, shoppingListId), eq(shoppingLists.userId, session?.user.id!))).leftJoin(shoppingProducts, eq(shoppingLists.id, shoppingProducts.shoppingListId));
