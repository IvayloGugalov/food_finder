...
export const createShoppingList = async (shoppingList: NewShoppingListParams) => {
  ...
  const newShoppingList = insertShoppingListSchema.parse({ ...shoppingList, userId: session?.user.id! });
  try {
...
export const updateShoppingList = async (id: ShoppingListId, shoppingList: UpdateShoppingListParams) => {
  const { session } = await getUserAuth();
  const { id: shoppingListId } = shoppingListIdSchema.parse({ id });
  const newShoppingList = updateShoppingListSchema.parse({ ...shoppingList, userId: session?.user.id! });
  try {
    const [s] =  await db
...
export const deleteShoppingList = async (id: ShoppingListId) => {
  ...
  const { id: shoppingListId } = shoppingListIdSchema.parse({ id });
  try {
    const [s] =  await db.delete(shoppingLists).where(and(eq(shoppingLists.id, shoppingListId!), eq(shoppingLists.userId, session?.user.id!)))
    .returning();
    return { shoppingList: s };
  } catch (err) {
    const message = (err as Error).message ?? "Error, please try again";
