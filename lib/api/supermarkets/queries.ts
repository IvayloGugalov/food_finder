...
export const getSupermarkets = async () => {
  ...
  const rows = await db.select().from(supermarkets).where(eq(supermarkets.userId, session?.user.id!));
  const s = rows
...
export const getSupermarketById = async (id: SupermarketId) => {
  ...
  const [row] = await db.select().from(supermarkets).where(and(eq(supermarkets.id, supermarketId), eq(supermarkets.userId, session?.user.id!)));
...
export const getSupermarketByIdWithProducts = async (id: SupermarketId) => {
  const { session } = await getUserAuth();
  const { id: supermarketId } = supermarketIdSchema.parse({ id });
  const rows = await db.select({ supermarket: supermarkets, product: products }).from(supermarkets).where(and(eq(supermarkets.id, supermarketId), eq(supermarkets.userId, session?.user.id!))).leftJoin(products, eq(supermarkets.id, products.supermarketId));
