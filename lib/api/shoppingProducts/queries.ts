...
export const getShoppingProducts = async () => {
  const rows = await db
    .select({ shoppingProduct: shoppingProducts, product: products, shoppingList: shoppingLists })
...
export const getShoppingProductByProductAndShoppingListId = async (productId: ProductId, shoppingListId: ShoppingListId) => {
  const { productId: ogProductId, shoppingListId: listId } = shoppingProductProductAndShoppingListIdSchema.parse({
    productId,
    shoppingListId,
  })
  const [row] = await db
    ...
    .from(shoppingProducts)
    .where(and(eq(shoppingProducts.productId, ogProductId), eq(shoppingProducts.shoppingListId, listId)))
  if (row === undefined) return {}
