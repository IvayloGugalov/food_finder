import { getShoppingProductById, getShoppingProducts } from "@/lib/api/shoppingProducts/queries";
...
import { createShoppingProduct, deleteShoppingProduct, updateShoppingProduct } from "@/lib/api/shoppingProducts/mutations";

export const shoppingProductsRouter = router({
  getShoppingProducts: publicProcedure.query(async () => {
    return getShoppingProducts();
  }),
  getShoppingProductById: publicProcedure.input(shoppingProductIdSchema).query(async ({ input }) => {
