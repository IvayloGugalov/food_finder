...
import { createShoppingList, deleteShoppingList, updateShoppingList } from "@/lib/api/shoppingLists/mutations";

export const shoppingListsRouter = router({
  getShoppingLists: publicProcedure.query(async () => {
    return getShoppingLists();
  }),
  getShoppingListById: publicProcedure.input(shoppingListIdSchema).query(async ({ input }) => {
