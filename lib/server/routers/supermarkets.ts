...
import { createSupermarket, deleteSupermarket, updateSupermarket } from "@/lib/api/supermarkets/mutations";

export const supermarketsRouter = router({
  getSupermarkets: publicProcedure.query(async () => {
    return getSupermarkets();
  }),
  getSupermarketById: publicProcedure.input(supermarketIdSchema).query(async ({ input }) => {
