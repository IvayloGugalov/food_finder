...
export default function ShoppingProductList({
  ...
  const { optimisticShoppingProducts, addOptimisticShoppingProduct } = useOptimisticShoppingProducts(
    shoppingProducts,
    products,
  shoppingLists 
  );
  ...
  return (
    ...
    >
      ...
      <Button variant={"link"} asChild>
        <Link href={ basePath + "/" + shoppingProduct.id }>
          Edit
        </Link>
      </Button>
    </li>
    ...
    <div className="text-center">
      ...
      <div className="mt-6">
        <Button onClick={() => openModal()}>
          <PlusIcon className="h-4" /> New Shopping Products </Button>
      </div>
    </div>
  );
};
