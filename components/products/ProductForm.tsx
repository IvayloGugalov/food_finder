...
,  const [validUntil, setValidUntil] = useState<Date | undefined>(
  ...
  const handleSubmit = async (data: FormData) => {
    ...
    const productParsed = await insertProductParams.safeParseAsync({ supermarketId, ...payload });
    ...
    <form action={handleSubmit} onChange={handleChange} className={"space-y-8"}>
      {/* Schema fields start */}
        ...
        <Label
          className={cn(
            "mb-2 inline-block",
            errors?.picUrl ? "text-destructive" : "",
          )}
        >
          Pic Url
        </Label>
        ...
        <Select defaultValue={product?.supermarketId} name="supermarketId">
          ...
          {supermarkets?.map((supermarket) => (
            <SelectItem key={supermarket.id} value={supermarket.id.toString()}>
              {supermarket.id}{/* TODO: Replace with a field from the supermarket model */}
            </SelectItem>
           ))}
          </SelectContent>
        </Select>
