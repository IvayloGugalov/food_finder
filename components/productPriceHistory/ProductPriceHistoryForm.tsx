...
const ProductPriceHistoryForm = ({
  ...
  const onSuccess = (
    action: Action,
    data?: { error: string; values: ProductPriceHistory }
  ) => {
  ...
  const handleSubmit = async (data: FormData) => {
    ...
    const productPriceHistoryParsed =
      await insertProductPriceHistoryParams.safeParseAsync({ productId, ...payload })
        ...
        <Popover>
          ...
          <PopoverTrigger asChild>
            <Button
              ...
              className={cn(
                'w-[240px] pl-3 text-left font-normal',
                !productPriceHistory?.weekDayEnd && 'text-muted-foreground'
              )}
            >
              {weekDayEnd ? (
      ...
      {editing ? (
        <Button
          ...
          onClick={() => {
            ...
            startMutation(async () => {
              addOptimistic &&
                addOptimistic({ action: 'delete', data: productPriceHistory })
              const error = await deleteProductPriceHistoryAction(
